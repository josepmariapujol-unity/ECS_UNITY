...
namespace Unity.Scenes
{
    ...
    public class SubScene : MonoBehaviour
    {
        ...
        void WarnDuplicate()
        {
            if (SceneAsset != null && !IsPartOfPrefab())
            {
                foreach (var subscene in m_AllSubScenes)
                {
                    if (subscene.SceneAsset == SceneAsset && !subscene.IsPartOfPrefab())
                    {
        ...
        {
            return PrefabUtility.IsPartOfAnyPrefab(gameObject) || (UnityEditor.Experimental.SceneManagement.PrefabStageUtility.GetPrefabStage(gameObject) != null);
        }

        internal bool IsInMainStage()
        {
            return StageUtility.GetStageHandle(gameObject) == StageUtility.GetMainStageHandle();
        }
#endif
