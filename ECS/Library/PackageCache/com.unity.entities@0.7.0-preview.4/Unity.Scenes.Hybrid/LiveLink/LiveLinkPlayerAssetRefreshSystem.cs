...
namespace Unity.Scenes
{
    ...
    class LiveLinkPlayerAssetRefreshSystem : ComponentSystem
    {
        ...
        unsafe void ReceiveSubScene(MessageEventArgs args)
        {
            ...
            if (!IsSubSceneAvailable(subSceneId, out var sceneMetaDataRef))
            {
                Debug.LogError("SubScene is missing artifacts!");
                return;
            }
            
            AddWaitForSubScene(subSceneId, sceneMetaDataRef);
        }
        ...
        unsafe void ReceiveResponseSubSceneTargetHash(MessageEventArgs args)
        {
            using (var subSceneAssets = args.ReceiveArray<ResolvedSubSceneID>())
            {
                foreach(var subSceneAsset in subSceneAssets)
                {
                    ...
                    if (IsSubSceneAvailable(subSceneAsset, out var sceneMetaDataRef))
                    {
                        ...
                        AddWaitForSubScene(subSceneAsset, sceneMetaDataRef);
                    }
                    else
                    {
        ...
        bool IsSubSceneAvailable(in ResolvedSubSceneID subSceneId, out BlobAssetReference<SceneMetaData> sceneMetaDataRef)
        {
            sceneMetaDataRef = BlobAssetReference<SceneMetaData>.Null;
            ...
            if (!BlobAssetReference<SceneMetaData>.TryRead(headerPath, SceneMetaDataSerializeUtility.CurrentFileFormatVersion, out sceneMetaDataRef))
            {
        ...
        void AddWaitForSubScene(in ResolvedSubSceneID subSceneId, in BlobAssetReference<SceneMetaData> sceneMetaDataRef)
        {
            ...
            ref SceneMetaData sceneMetaData = ref sceneMetaDataRef.Value;
            var runtimeGlobalObjectIdSet = new HashSet<RuntimeGlobalObjectId>();
            ...
            for (int i = 0; i < sceneMetaData.Sections.Length; i++)
            {
                if (sceneMetaData.Sections[i].ObjectReferenceCount != 0)
                {
                    ...
                    using (var reader = new StreamBinaryReader(refObjGuidsPath))
                    {
                        ...
                        {
                            runtimeGlobalObjectIdSet.Add(runtimeGlobalObjectId);
                        }
            ...
            var waitingSubScene = new WaitingSubScene {TargetHash = subSceneId.TargetHash};

            if (runtimeGlobalObjectIdSet.Count > 0)
            {
                int i = 0;
                waitingSubScene.RuntimeGlobalObjectIds = new NativeArray<RuntimeGlobalObjectId>(runtimeGlobalObjectIdSet.Count, Allocator.Persistent);
                foreach (var runtimeGlobalObjectId in runtimeGlobalObjectIdSet)
                    waitingSubScene.RuntimeGlobalObjectIds[i++] = runtimeGlobalObjectId;
            }
