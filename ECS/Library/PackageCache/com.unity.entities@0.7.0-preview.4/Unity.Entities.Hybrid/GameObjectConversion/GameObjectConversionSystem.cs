...
public abstract partial class GameObjectConversionSystem : ComponentSystem
{
...
#if UNITY_EDITOR
    /// <summary>
    /// Get an <see cref="Unity.Platforms.Build.IBuildComponent"/> of the given type from the current build configuration. If there are
    ...
    /// </summary>
    public T GetBuildConfigurationComponent<T>() where T : Unity.Platforms.Build.IBuildComponent => m_MappingSystem.GetBuildConfigurationComponent<T>();

    /// <summary>
    /// Try to get an <see cref="Unity.Platforms.Build.IBuildComponent"/> of the given type from the current build configuration. If there are
    ...
    /// </summary>
    public bool TryGetBuildConfigurationComponent<T>(out T component) where T : Unity.Platforms.Build.IBuildComponent => m_MappingSystem.TryGetBuildConfigurationComponent(out component);
    ...
    public T GetBuildSettingsComponent<T>() where T : Unity.Platforms.Build.IBuildComponent => m_MappingSystem.GetBuildConfigurationComponent<T>();
    ...
    public bool TryGetBuildSettingsComponent<T>(out T component) where T : Unity.Platforms.Build.IBuildComponent => m_MappingSystem.TryGetBuildConfigurationComponent(out component);
