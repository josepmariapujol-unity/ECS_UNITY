...
{
    internal enum SubSceneComponentValidity
    {
        Valid,
        SubScenesInPrefabsIsNotSupported,
        SubScenesOutsideMainStageIsNotSupported,
        DuplicateSubScenesIsNotSupported,
        NestedSubScenesIsNotSuppported,
    }

    internal static class SubSceneInspectorUtility
    {
        internal static SubSceneComponentValidity GetSubSceneComponentValidity(SubScene subScene)
        {
            if (subScene.IsPartOfPrefab())
            {
                return SubSceneComponentValidity.SubScenesInPrefabsIsNotSupported;
            }

            if (!subScene.IsInMainStage())
            {
                return SubSceneComponentValidity.SubScenesOutsideMainStageIsNotSupported;
            }

            var isNestedSubScene = subScene.gameObject.scene.isSubScene;
            if (isNestedSubScene)
            {
                return SubSceneComponentValidity.NestedSubScenesIsNotSuppported;
            }

            bool hasDuplicates = subScene.SceneAsset != null && (SubScene.AllSubScenes.Count(s => (s.SceneAsset == subScene.SceneAsset) && !s.IsPartOfPrefab()) > 1);
            if (hasDuplicates)
            {
                return SubSceneComponentValidity.DuplicateSubScenesIsNotSupported;
            }

            return SubSceneComponentValidity.Valid;
        }
