...
namespace Unity.Scenes.Editor
{
    class SubSceneContextMenu
    {
        ...
        {
            var targetGameObject = args.target;
            var parentScene = targetGameObject != null ? targetGameObject.scene : args.parentScene;
            ...
            GameObject[] topLevelGameObjects = null;
        ...
        static void OnMenuItemForNewSubScene(object userData)
        {
            ...
            if (args.target != null && args.target.scene.isSubScene)
            {
                EditorUtility.DisplayDialog("Nesting Sub Scenes Not Supported", "Creating the Sub Scene here will result in a nested Sub Scene which are not yet supported.", "OK");
                return;
            }
            ...
            var dstFolder = Path.Combine(Path.GetDirectoryName(parentScene.path), parentSceneName);
            var baseName = "New Sub Scene";
            var dstPath = GetActualPathName(Path.Combine(dstFolder, baseName + ".unity"));
            if (Directory.Exists(dstFolder))
                dstPath = AssetDatabase.GenerateUniqueAssetPath(dstPath);

            var renameWindowArgs = new RenameWindowArgs
            {
                windowTitle = EditorGUIUtility.TrTextContent("Create New Sub Scene").text,
                infoTextAboveTextField = EditorGUIUtility.TrTextContent("Please enter the name of the new Scene file").text,
                infoTextBelowTextField = EditorGUIUtility.TrTextContent("Parent Scene: ").text + parentSceneName,
                okButttonText = EditorGUIUtility.TrTextContent("Create").text,
                cancelButttonText = EditorGUIUtility.TrTextContent("Cancel").text,
                startName = Path.GetFileNameWithoutExtension(dstPath),
                resultCallback = RenameWindowResultCallback,
                checkNameCallback = CheckNameCallback,
                userData = args
            };
            RenameWindow.Show(renameWindowArgs);
        }
        ...
        static void RenameWindowResultCallback(bool userAccepted, string name, object userData)
        {
            if (!userAccepted)
                return;

            CreateNewSubScene(name, (NewSubSceneArgs)userData, InteractionMode.UserAction);
        }
