...
namespace Unity.Scenes.Editor
{
    ...
    class SubSceneInspector : UnityEditor.Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            var subSceneValidity = SubSceneInspectorUtility.GetSubSceneComponentValidity(subScene);
            switch (subSceneValidity)
            {
                case SubSceneComponentValidity.Valid:
                    // Nop
                    break;

                case SubSceneComponentValidity.SubScenesOutsideMainStageIsNotSupported:
                    EditorGUILayout.HelpBox($"Only Sub Scenes in the Main Stage are supported.", MessageType.Warning, true);
                    EditorGUILayout.Space();
                    return;

                case SubSceneComponentValidity.SubScenesInPrefabsIsNotSupported:
                    EditorGUILayout.HelpBox($"Sub Scenes in Prefabs are not supported.", MessageType.Warning, true);
                    EditorGUILayout.Space();
                    return;
                    
                case SubSceneComponentValidity.DuplicateSubScenesIsNotSupported:
                    // Handled below because we want to show the Inspector so the user can fix up the Scene Asset reference duplicate
                    break;

                case SubSceneComponentValidity.NestedSubScenesIsNotSuppported:
            ...
            if (subSceneValidity == SubSceneComponentValidity.DuplicateSubScenesIsNotSupported)
            {
