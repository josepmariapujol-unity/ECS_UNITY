...
namespace Unity.Scenes.Editor
{
    class LiveLinkAssetBundleBuildSystem : ScriptableSingleton<LiveLinkAssetBundleBuildSystem>
    {
        ...
        unsafe void SendBuildArtifact(string artifactPath, int playerId)
        {
        ...
        void SendSubScene(ResolvedSubSceneID subSceneId, int playerId)
        {
            ...
            for (int j = 0; j != sceneMetaData.Sections.Length; j++)
            {
                ...
                var refObjsPath = EntityScenesPaths.GetLoadPathFromArtifactPaths(paths, EntityScenesPaths.PathType.EntitiesUnityObjectRefGuids, sectionIndex);
                
                SendBuildArtifact(binaryPath, playerId);
                SendBuildArtifact(refObjsPath, playerId);
            }

            EditorConnection.instance.Send(LiveLinkMsg.ResponseSubSceneForGUID, subSceneId, playerId);
        }		
